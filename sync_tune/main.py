...
class ConnectionManager:
	...
	def disconnect(self, websocket: WebSocket):
		self.connected_clients = [client for client in self.connected_clients if client['websocket'] != websocket]
	...
	async def send_welcome_message(self, websocket: WebSocket, username: str):
		for client in self.connected_clients:
			if client['websocket'] != websocket:
				await client['websocket'].send_text(f"Пользователь, {username} присоединился в плеер!")
